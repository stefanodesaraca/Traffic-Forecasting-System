...
def savePlots(plotFunction):
    ...
    def wrapper(*args, **kwargs):
        ...
        if checkPlots(plotsNames, generatedPlots) is True:
            ...
            print(
                f"\033[91mExporting the plots wasn't possible, here's the data types obtained by the decorator: PlotNames: {type(plotsNames)}, Generated Plots (could be a list of plots): {type(generatedPlots)}, File Path: {type(filePath)}\033[0m"
            )
...
def analyze_volumes(volumes: pd.DataFrame) -> None:
    ...
    percentile_25_by_year = volumes.groupby(volumes["year"], as_index=False)[
        "volume"
    ].quantile(0.25)
    percentile_50_by_year = volumes.groupby(volumes["year"], as_index=False)[
        "volume"
    ].quantile(0.50)
    percentile_75_by_year = volumes.groupby(volumes["year"], as_index=False)[
        "volume"
    ].quantile(0.75)
    percentile_95_by_year = volumes.groupby(volumes["year"], as_index=False)[
        "volume"
    ].quantile(0.95)
    percentile_99_by_year = volumes.groupby(volumes["year"], as_index=False)[
        "volume"
    ].quantile(0.99)
    ...
    for y in sorted(external_values_by_year.keys()):
        ...
        print(
            f"Volumes standard variance for year {y}: ",
    ...
    dates = [
        f"{y}-{m}-{d}"
        for y, m, d in zip(volumes["year"], volumes["month"], volumes["day"])
    ]
    ...
    volumes = volumes.drop(
        columns=["date"]
    )  # TODO CHECK IF THIS IS REMOVABLE BY TESTING THE FUNCTION ITSELF
...
def analyze_avg_speeds(speeds: pd.DataFrame) -> None:
    ...
    percentile_25_by_year = speeds.groupby(speeds["year"], as_index=False)[
        "mean_speed"
    ].quantile(0.25)
    percentile_50_by_year = speeds.groupby(speeds["year"], as_index=False)[
        "mean_speed"
    ].quantile(0.50)
    percentile_75_by_year = speeds.groupby(speeds["year"], as_index=False)[
        "mean_speed"
    ].quantile(0.75)
    percentile_95_by_year = speeds.groupby(speeds["year"], as_index=False)[
        "mean_speed"
    ].quantile(0.95)
    percentile_99_by_year = speeds.groupby(speeds["year"], as_index=False)[
        "mean_speed"
    ].quantile(0.99)
    ...
    print(
        "Average speeds standard variance: ", np.round(np.var(speeds["mean_speed"]), 2)
    )
    print("\n")
