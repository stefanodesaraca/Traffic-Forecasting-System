...
class AIODBBroker:
    ...
    async def send_sql_async(
        self,
        sql: str,
        single: bool = False,
        many: bool = False,
        many_values: list[tuple[Any, ...]] | None = None,
    ) -> Any:
        async with postgres_conn_async(
            user=self._db_user,
    ...
    async def get_volume_date_boundaries_async(self) -> dict[str, Any]:
        async with postgres_conn_async(
            ...
            async with conn.transaction():
                ...
                """)
                return {
                    "min": result["volume_start_date"],
                    "max": result["volume_end_date"],
                }  # Respectively: min and max
    ...
    async def get_mean_speed_date_boundaries_async(self) -> dict[str, Any]:
        async with postgres_conn_async(
            ...
            async with conn.transaction():
                ...
                """)
                return {
                    "min": result["mean_speed_start_date"],
                    "max": result["mean_speed_end_date"],
                }  # Respectively: min and max

    @cached_async()
    async def get_road_categories_async(
        self, name_as_key: bool = False
    ) -> dict[str, str]:
        async with postgres_conn_async(
            user=self._db_user,
            password=self._db_password,
            ...
            async with conn.transaction():
                if not name_as_key:
                    return {
                        ...
                        for row in (
                            await conn.fetch(
                                f'SELECT id, name FROM "{ProjectTables.RoadCategories.value}";'
                            )
                        )
                    }
                return {
    ...
    async def set_forecasting_horizon_async(
        self,
        forecasting_window_size: PositiveInt = GlobalDefinitions.DEFAULT_MAX_FORECASTING_WINDOW_SIZE,
    ) -> None:
        """
        Parameters:
        ...
        Returns:
            None
        """
        max_forecasting_window_size: int = max(
            GlobalDefinitions.DEFAULT_MAX_FORECASTING_WINDOW_SIZE,
        ...
        if target == GlobalDefinitions.VOLUME:
            last_available_data_dt = (await self.get_volume_date_boundaries_async())[
                "max"
            ]
        elif target == GlobalDefinitions.MEAN_SPEED:
            last_available_data_dt = (
                await self.get_mean_speed_date_boundaries_async()
            )["max"]
        else:
        ...
        print(
            "Maximum settable date: ",
            last_available_data_dt
            + relativedelta(
                last_available_data_dt,
                days=GlobalDefinitions.DEFAULT_MAX_FORECASTING_WINDOW_SIZE,
            ),
        )

        horizon = datetime.datetime.strptime(
            input("Insert forecasting horizon (YYYY-MM-DDTHH): ")
            + ":00:00"
        ...
        assert horizon > last_available_data_dt, (
        ...
        assert (horizon - last_available_data_dt).days <= max_forecasting_window_size, (
    ...
    async def get_forecasting_horizon_async(self, target: str) -> datetime.datetime:
        ...
        return (
            await self.send_sql_async(f"""SELECT ("config" ->> '{target}_forecasting_horizon')::timestamptz AS horizon
...
class DBBroker:
    ...
    @contextmanager
    def PostgresConnectionCursor(
        self,
        conn: psycopg.Connection,
        query: LiteralString,
        row_factory: Literal["tuple_row", "dict_row"] = "dict_row",
    ...
    def send_sql(
        ...
        many_values: list[tuple[Any, ...]] | None = None,
        row_factory: Literal["tuple_row", "dict_row"] = "dict_row",
        execute_args: list[Any] | dict[str, Any] | None = None,
    ) -> Any:
        with postgres_conn(
            user=self._db_user,
            password=self._db_password,
            ...
            with conn.cursor(
                row_factory=RowFactories.factories.get(row_factory)
            ) as cur:
                if any(
                    sql.lstrip().startswith(prefix)
                    for prefix in ["SELECT", "WITH", "select", "with"]
    ...
    def get_stream(
        ...
        filters: tuple[Any] | tuple[str, ...] | tuple[list[Any], ...],
        row_factory: Literal["tuple_row", "dict_row"] = "dict_row",
    ) -> Generator:
        if "SELECT" not in sql:
            raise WrongSQLStatementError(
                "Cannot return a data stream from a non selective statement (SELECT)"
            )
        with postgres_conn(
            user=self._db_user,
            password=self._db_password,
    ...
    def get_trp_ids(
        ...
        with postgres_conn(
            ...
            with self.PostgresConnectionCursor(
                query=f"""
                    SELECT "id" FROM "{ProjectTables.TrafficRegistrationPoints.value}"
                    {'WHERE "road_category" = ANY(%s)' if road_category_filter else ""};
                 """,
                params=tuple(to_pg_array(f) for f in [road_category_filter] if f),
                conn=conn,
            ) as cur:
                return cur.fetchall()

    def get_trp_ids_by_road_category(
        self,
        ...
        with postgres_conn(
            ...
            with self.PostgresConnectionCursor(
                query=f"""SELECT json_object_agg("road_category", "ids") AS result
                                                         FROM (
                                                      ...
                                                      """,
                conn=conn,
                params=tuple(to_pg_array(f) for f in [county_ids_filter] if f),
            ) as cur:
    ...
    @cached()
    def get_volume_date_boundaries(
        self, trp_id_filter: list[str] | None = None
    ) -> dict[str, Any]:
        ...
        with postgres_conn(
            ...
            with self.PostgresConnectionCursor(
                ...
                result = cur.fetchone()
                return {
                    "min": result[f"{GlobalDefinitions.VOLUME}_start_date"],
                    "max": result[f"{GlobalDefinitions.VOLUME}_end_date"],
                }  # Respectively: min and max

    @cached()
    def get_mean_speed_date_boundaries(
        self, trp_id_filter: list[str] | None = None
    ) -> dict[str, Any]:
        with postgres_conn(
            user=self._db_user,
            ...
            with self.PostgresConnectionCursor(
                ...
                result = cur.fetchone()
                return {
                    "min": result[f"{GlobalDefinitions.MEAN_SPEED}_start_date"],
                    "max": result[f"{GlobalDefinitions.MEAN_SPEED}_end_date"],
                }  # Respectively: min and max

    def get_all_trps_metadata(
        self, has_volume_filter: bool = False, has_mean_speed_filter: bool = False
    ...
    def get_trained_model_objects(
        self, target: str, road_category: str
    ) -> list[dict[str, Any]]:
        with postgres_conn(
            user=self._db_user,
            password=self._db_password,
    ...
    def get_trained_models(
        self, target: str | None = None, road_category: str | None = None
    ) -> dict[str, Any]:
        if target:
    ...
    def get_municipality_trps(
        self, municipality_id: PositiveInt
    ) -> list[dict[str, Any]]:
        return self.send_sql(
            f"""
    ...
    def get_municipality_geometry(
        self, municipality_id: PositiveInt, as_wgs84: bool | None = None
    ) -> BaseGeometry:
        return wkt.loads(
            self.send_sql(
                f"""
    ...
    def update_model_best_gridsearch_params(
        self, best_gridsearch_params: list[dict[str, Any]]
    ) -> None:
        """
...
class AIODBManagerBroker:
    ...
    async def init(self, auto_project_setup: bool = True) -> None:
        await (await self._get_db_manager_async()).init(
            auto_project_setup=auto_project_setup
        )
        return None

    async def create_project(
        self, name: str, lang: str = "en", auto_project_setup: bool = True
    ) -> None:
        await (await self._get_db_manager_async()).create_project(
            name=name, lang=lang, auto_project_setup=auto_project_setup
        )
        return None
    ...
    async def insert_models(self) -> None:
        async with postgres_conn_async(
            user=self._tfs_user,
            password=self._tfs_password,
            dbname=(await self.get_current_project()).get("name", None),
            host=self._db_host,
        ) as conn:
    ...
    async def update_municipalities_geometries(self) -> None:
        municipality_aux_data = await asyncio.to_thread(
            pd.read_csv,
            GlobalDefinitions.MUNICIPALITIES_AUXILIARY_DATA,
            sep=";",
            encoding="utf-8",
        )
        async with postgres_conn_async(
            ...
            for m in muni:
                muni_row = municipality_aux_data.query(
                    f"`EGS.KOMMUNENUMMER.11769` == {m}"
                )
                geom_value = (
                    muni_row["GEO.GEOMETRI"].iloc[0] if not muni_row.empty else None
                )
                await conn.execute(
                    f"""
