...
warnings.filterwarnings("ignore", module="distributed.shuffle")
...
class RoadNetwork:
    def __init__(
        ...
        self._network: nx.DiGraph | None = (
            None  # TODO CORRECTLY DEFINE ROAD DIRECTIONS, BUT WITH AN UNDIRECTED GRAPH EVERYTHING WORKS CORRECTLY. CHECK IF MULTI DIGRAPH WORKS
        )

        if not network_binary:
            self._network = nx.DiGraph(
                **{"network_id": self.network_id, "name": self.name}
            )
        else:
    ...
    def _get_minkowski_dist(u: tuple[Any, ...], v: tuple[Any, ...], G: nx.DiGraph):
        ...
        return minkowski(
            [u_easting, u_northing], [v_easting, v_northing], p=2.0
        )  # TODO 1.0 if G.edges[u, v]["road_category"] in GlobalDefinitions.HIGH_SPEED_ROAD_CATEGORIES else

    def _get_neighbor_trps(
        self, link_id: str, buffer_zone_radius: PositiveInt = 3500
    ) -> list[dict[str, Any]]:
    ...
    def _get_link_aggregated_traffic_data(
        ...
        return self._db_broker.send_sql(
            ...
            execute_args=[
                link_id,
                county_avg_weight,
                municipality_avg_weight,
                road_category_avg_weight,
                county_avg_weight,
                municipality_avg_weight,
                road_category_avg_weight,
            ],
    ...
    def _get_trp_predictions(
        ...
        models = {
            m["name"]: pickle.loads(m["pickle_object"])
            for m in self._db_broker.get_trained_model_objects(
                target=target, road_category=road_category
            )
        }  # WARNING: If no models exist for a specific road category this could end up being an empty dictionary
        return MLPredictionPipeline(
            trp_id=trp_id,
            target=target,
            ...
            loader=self._loader,
            db_broker=self._db_broker,
        ).start(training_mode=1, lags=lags)  # TODO CUSTOMIZE TRAINING MODE

    def load_nodes(self) -> None:
        all(
            self._network.add_nodes_from(
                (row.to_dict().pop("node_id"), row) for _, row in partition.iterrows()
            )
            for partition in self._loader.get_nodes(lat=True, lon=True).partitions
        )
        return None
    ...
    def load_links(
        ...
        all(
            self._network.add_edges_from(
                (
                    row["start_traffic_node_id"],
                    row["end_traffic_node_id"],
                    {
                        k: v
                        for k, v in row.to_dict().items()
                        if k not in ["start_traffic_node_id", "end_traffic_node_id"]
                    },
                )
    ...
    def _get_path_edges(self, p: list[str]) -> list[tuple[str, str, Mapping[str, Any]]]:
        return [
            (u, v, self._network.get_edge_data(u, v)) for u, v in zip(p, p[1:])
        ]  # TODO TEST IF THIS WORKS WITH A GENERATOR AS WELL CALLING THE find_route() METHOD TWICE IN THE SAME RUNTIME

    def _get_trps_per_edge(
        self,
        edges: list[tuple[str, str, Mapping[str, Any]]],
        trp_research_buffer_radius: PositiveInt,
    ) -> dict[str, list[dict[str, Any]]]:
        trps_per_edge = {}
        while True:
            for e in edges:
                trps_per_edge[e[2]["link_id"]] = self._get_neighbor_trps(
                    e[2]["link_id"], trp_research_buffer_radius
                )
    ...
    @staticmethod
    def _get_ok_structured_data(
        y_pred: dict[str, dict[str, dd.DataFrame] | Any],
        ...
        return dd.from_pandas(
            pd.DataFrame(
                [
                    ...
                    for _, row in trp_data[f"{target}_preds"]
                    .loc[trp_data[f"{target}_preds"]["zoned_dt_iso"] == horizon]
                    .iterrows()
                ]
            ),
            npartitions=1,
    ...
    def _get_ordinary_kriging(
        ...
        ok_df = self._get_ok_structured_data(
            y_pred=y_pred, horizon=horizon, target=target
        )
    ...
    @staticmethod
    def _ok_interpolate(
        ordinary_kriging_obj: OrdinaryKriging,
        x_coords: list[float | np.floating],
        y_coords: list[float | np.floating],
        style: Literal["grid", "points"],
    ) -> Any:
        if not len(x_coords) == len(y_coords):
            raise ValueError(
                "There must be exactly the same number of pairs of coordinates"
            )
        return ordinary_kriging_obj.execute(
            style=style, xpoints=x_coords, ypoints=y_coords
        )
    ...
    @staticmethod
    def _get_edge_weighting(
        edge_start: str, edge_end: str, attrs: dict | Mapping[str, Any]
    ) -> PositiveFloat | None:
        ...
        speed_factor = (
            ...
            ((highest_speed_limit / 100) * 85) * 0.20 + highest_speed_limit * 0.20
        )
        lowest_speed_contribution = lowest_speed_limit * 0.05
        # Contribution of the lowest speed limit

        return abs(
            travel_time_factor + lane_factor + speed_factor + lowest_speed_contribution
        )  # Can't return a potentially negative weight

    def _update_trps_preds(
        self, targets: list[str], lags: list[int], model: str
    ) -> None:
        self.trps_along_sp_preds.update(
            **{
                ...
                for trp in list(
                    ...
                    if trp["id"] not in self.trps_along_sp_preds.keys()
                )  # Only calculating the predictions for the TRPs which hadn't seen their data predict yet, #TODO CHECK CONTENT: list(filter(lambda x: x not in self.trps_along_sp_preds.keys(), self.trps_along_sp))
    ...
    def find_route(
        ...
        if not horizon.strftime(
            GlobalDefinitions.DT_ISO_TZ_FORMAT
        ):  # Must strictly be not zoned
            raise ValueError(
                f"The horizon value must be in {GlobalDefinitions.DT_ISO_TZ_FORMAT} format"
            )

        if not any([use_volume, use_mean_speed]):
            raise ValueError(
                f"At least one of use_{GlobalDefinitions.VOLUME} or use_{GlobalDefinitions.MEAN_SPEED} must be set to True"
            )
        ...
        if has_only_public_transport_lanes:
            self._network.remove_edges_from(
                has_only_public_transport_lanes_edges := [
                    (u, v)
                    for u, v, attrs in self._network.edges(data=True)
                    if attrs["has_only_public_transport_lanes"] < 3
                ]
            )
        if has_toll_stations:
            self._network.remove_edges_from(
                has_toll_stations_edges := [
                    (u, v)
                    for u, v, attrs in self._network.edges(data=True)
                    if attrs["has_toll_stations"] < 3
                ]
            )
        if has_ferry_routes:
            self._network.remove_edges_from(
                has_ferry_routes_edges := [
                    (u, v)
                    for u, v, attrs in self._network.edges(data=True)
                    if attrs["has_ferry_routes"] < 3
                ]
            )

        # Removing isolated nodes since they're unreachable
        self._network.remove_nodes_from(
            unreachable := list(i for i in nx.isolates(self._network))
        )
        ...
        try:
            for p in range(max_iter):
                ...
                sp = self._get_shortest_path(
                    source=source,
                    destination=destination,
                    heuristic=heuristic,
                    weight=self._get_edge_weighting,
                )
                print(sp)
                ...
                trps_per_edge = self._get_trps_per_edge(
                    edges=sp_edges,
                    trp_research_buffer_radius=trp_research_buffer_radius,
                )
                unique_trps = {
                    tuple(d.items())
                    for d in chain.from_iterable(trps_per_edge.values())
                }
                ...
                self.trps_along_sp = [
                    dict(t) for t in unique_trps
                ]  # A set of dictionary where each dict is a TRP with its metadata (id, road_category, etc.)
                ...
                total_sp_length = sum(
                    self._network.edges[u, v]["length"] for u, v in zip(sp[:-1], sp[1:])
                )
                print(
                    "Route traits length: ",
                    [
                ...
                paths.update(
                    {
                        str(p): {
                            ...
                            "forecasted_travel_time": round(
                                (
                                    (
                                        total_sp_length
                                        / (
                                            ((average_highest_speed_limit / 100) * 90)
                                            / 3.6
                                        )
                                    )
                                    / 60
                ...
                link_agg_data = dict(
                    (link_id, self._get_link_aggregated_traffic_data(link_id=link_id))
                    for link_id in trps_per_edge.keys()
                ...
                for target in targets:
                    ok, variogram_plot = self._get_ordinary_kriging(
                        y_pred=self.trps_along_sp_preds,
                        horizon=horizon,
                        target=target,
                        verbose=True,
                    )
                    ...
                    variogram_plot = self._edit_variogram_plot(
                        fig=variogram_plot, target=target
                    )

                    line_predictions[f"{target}_interpolated_value"] = (
                        z_interpolated_vals
                    )
                    line_predictions[f"{target}_variance"] = kriging_variance

                    line_predictions[f"link_avg_{target}"] = line_predictions[
                        "link_id"
                    ].map(
                        lambda link_id: link_agg_data[link_id][f"weighted_avg_{target}"]
                    )

                    # Difference (point prediction - link-level avg)
                    line_predictions[f"{target}_diff_from_avg"] = (
                        line_predictions[f"{target}_interpolated_value"]
                        - line_predictions[f"link_avg_{target}"]
                    )
                    ...
                    high_traffic_perc = (
                        line_predictions[f"{target}_traffic_class"]
                        .isin(
                            [TrafficClasses.HIGH_AVERAGE.name, TrafficClasses.HIGH.name]
                        )
                        .mean()
                        * 100
                    )  # Calculating the percentage of the total path which has a traffic level above HIGH_AVERAGE
                ...
                line_predictions["traffic_class"] = line_predictions[
                    traffic_class_cols
                ].apply(
                    lambda row: max(row, key=lambda x: TRAFFIC_PRIORITIES[x]), axis=1
                )
                ...
                if any(
                    paths[str(p)].get(f"{target}_high_traffic_perc", 0) > 50
                    for target in targets
                ):
                    trp_research_buffer_radius += (
                        2000  # Incrementing the TRP research buffer radius
                    )

                    sp_edges_weight = [
                        (u, v, self._get_edge_weighting(u, v, data))
                        for u, v, data in sp_edges
                    ]

                    # Sort by weight (descending) and pick the top-n heaviest nodes to remove them
                    n = max(
                        1, len(sp_edges)
                    )  # Maximum number of heaviest edges to remove
                    removed_edges[str(p)] = [
                        (u, v, self._network.edges[u, v].copy())
                        for u, v, w in sorted(
                            sp_edges_weight, key=lambda x: x[2], reverse=True
                        )[:n]
                    ...
                    self._network.remove_edges_from(
                        [(u, v) for u, v, _ in removed_edges[str(p)]]
                    )  # Adopting this method since remove_edges_from accepts (u, v) tuples without attributes, but we need to keep attributes for the future re-insertion in the graph of ALL the nodes removed during the iterations
    ...
    @staticmethod
    def _create_map(
        lat_init: float | np.floating,
        lon_init: float | np.floating,
        zoom: PositiveInt = 8,
        tiles: str = FoliumMapTiles.OPEN_STREET_MAPS.value,
    ) -> folium.Map:
        return folium.Map(location=[lat_init, lon_init], tiles=tiles, zoom_start=zoom)

    @staticmethod
    def _add_marker(
        folium_obj: folium.Map | folium.FeatureGroup,
        marker_lat: float | np.floating,
        marker_lon: float | np.floating,
    ...
    def _add_line(
        folium_obj: folium.Map | folium.FeatureGroup,
        locations: list[list[float | np.floating]],
        color: str,
        weight: float,
    ...
    def _get_route_start_end_nodes(
        self, route: dict[str, Any]
    ) -> tuple[dict[str, Any], dict[str, Any]]:
        return self._network.nodes[route["path"][0]], self._network.nodes[
            route["path"][-1]
        ]
    ...
    def _get_route_map_layers(self, route: dict[str, Any]) -> list[folium.FeatureGroup]:
        ...
        # Adding source node
        self._add_marker(
            folium_obj=steps_layer,
            marker_lat=path_start_node["lat"],
            marker_lon=path_start_node["lon"],
            popup="Start",
            icon=folium.Icon(
                icon=IconStyles.SOURCE_NODE_STYLE.value["icon"],
                icon_color=IconStyles.SOURCE_NODE_STYLE.value["icon_color"],
            ),
        )
        # Adding destination node
        self._add_marker(
            folium_obj=steps_layer,
            marker_lat=path_end_node["lat"],
            marker_lon=path_end_node["lon"],
            popup="Destination",
            icon=folium.Icon(
                icon=IconStyles.DESTINATION_NODE_STYLE.value["icon"],
                icon_color=IconStyles.DESTINATION_NODE_STYLE.value["icon_color"],
            ),
        )
        ...
        for trp in route["trps_along_path"]:
            self._add_marker(
                folium_obj=trps_layer,
                marker_lat=trp["lat"],
                marker_lon=trp["lon"],
                popup=trp["id"],
                icon=folium.Icon(
                    icon=IconStyles.TRP_LINK_STYLE.value["icon"],
                    icon_color=IconStyles.TRP_LINK_STYLE.value["icon_color"],
                ),
            )

        return [steps_layer, edges_layer, trps_layer]

    def _get_traffic_map_layers(
        self,
        trps: list[dict[str, Any]],
        municipality_geom: wkt,
        municipality_geom_color: str = "blue",
        municipality_geom_fill_color: str = "blue",
        municipality_fill_opacity: PositiveFloat = 0.3,
    ) -> list[folium.FeatureGroup]:
        municipality_geom_layer = folium.FeatureGroup("municipality_geom")
        trps_layer = folium.FeatureGroup("trps")

        coords = [
            (lat, lon) for lon, lat in municipality_geom.exterior.coords
        ]  # Must swap coordinates order since shapely works with x and y (longitude and latitude) and folium works with latitude and longitude
        folium.Polygon(
            locations=coords,
    ...
    @staticmethod
    def _get_layers_assembly(
        map_obj: folium.Map, layers: list[folium.FeatureGroup]
    ) -> folium.Map:
    ...
    def draw_route(
        ...
        zoom_init: PositiveInt | None = None,
    ) -> folium.Map:
        ...
        route_map = self._create_map(
            lat_init=map_loc_init[0],
            lon_init=map_loc_init[1],
    ...
    def draw_routes(
        ...
        if map_loc_init is None:
            map_loc_init = [
                (sn["lat"], sn["lon"])
                for sn, _ in [
                    tuple(self._get_route_start_end_nodes(route=r))
        ...
        return self._get_layers_assembly(
            map_obj=self._create_map(
                lat_init=lat_init,
                lon_init=lon_init,
                zoom=zoom_init or MapDefaultConfigs.ZOOM.value,
                tiles=tiles or FoliumMapTiles.OPEN_STREET_MAPS.value,
            ),  # The map where to add all layers
            layers=list(
                chain.from_iterable(
                    self._get_route_map_layers(route=r) for r in routes.values()
                )
            ),  # Map layers
        )

    def _get_municipality_id_preds(
        self, municipality_id: PositiveInt, target: str, model: str
    ) -> dict[str, dict[str, dd.DataFrame]]:
        return {
            trp["id"]: {
                f"{target}_preds": self._get_trp_predictions(
                    trp_id=trp["id"],
            ...
            for trp in self._db_broker.get_municipality_trps(
                municipality_id=municipality_id
            )
        }

    def _get_municipality_traffic_heatmap(
        self,
        municipality_id: PositiveInt,
        horizon: datetime.datetime,
        target: str,
        model: str,
        zoom_init: PositiveInt | None = None,
        ...
        municipality_geom = self._db_broker.get_municipality_geometry(
            municipality_id=municipality_id, as_wgs84=True
        )
        ...
        ok, variogram_plot = self._get_ordinary_kriging(
            y_pred=self._get_municipality_id_preds(
                municipality_id=municipality_id, target=target, model=model
            ),
            horizon=horizon,
            target=target,
        )
        ...
        municipality_traffic_map = self._create_map(
            lat_init=municipality_geom_center.y,
            lon_init=municipality_geom_center.x,
            zoom=zoom_init or MapDefaultConfigs.ZOOM.value,
            tiles=tiles or FoliumMapTiles.OPEN_STREET_MAPS.value,
        )
        municipality_traffic_map = self._get_layers_assembly(
            municipality_traffic_map,
            self._get_traffic_map_layers(
        ...
        grid_interpolation_viz = ax.imshow(
            z_interpolated_vals,
            extent=(min(gridx), max(gridx), min(gridy), max(gridy)),
            origin="lower",
        ...
        buf = io.BytesIO()
        fig.savefig(
            buf, format="png", pad_inches=0, transparent=True
        )  # TODO TRY WITHOUT bbox_...
    ...
    def draw_municipality_traffic_volume_heatmap(
        self,
        municipality_id: PositiveInt,
        horizon: datetime.datetime,
        model: str = "HistGradientBoostingRegressor",
        ...
        return self._get_municipality_traffic_heatmap(
            municipality_id=municipality_id,
            horizon=horizon,
    ...
    def draw_municipality_traffic_mean_speed_heatmap(
        self,
        municipality_id: PositiveInt,
        horizon: datetime.datetime,
        model: str = "HistGradientBoostingRegressor",
    ...
    def to_db(self) -> None:
        ...
        """,
            execute_args=[
                ...
                pickle.dumps(self._network, pickle.HIGHEST_PROTOCOL),
            ],
        )
        return None
    ...
    def save_graph_svg(self) -> tuple[plt, str]:
        ...
        nx.draw(
            ...
            edge_color=[
                ...
                if self._network.nodes[node].get("has_trps")
                else "blue"
