...
class MLPreprocessingPipeline:
    ...
    @staticmethod
    def _add_lag_features(data: pd.DataFrame, target: str, lags: list[PositiveInt]) -> dd.DataFrame:
        for lag in lags:
    ...
    def preprocess_volume(self,
        ...
        data = data.map_partitions(self._add_utm_columns).persist() #Convert latitude and longitude into the appropriate UTM zone eastings and northings to maintain an accurate ratio when moving across the zone and not having distortions like in WGS84 coordinates where the closer to the poles the heavier distortions come into place
