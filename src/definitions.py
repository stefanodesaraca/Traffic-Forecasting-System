...
class GlobalDefinitions(BaseModel):
    ...
    VOLUME_INGESTION_FIELDS: ClassVar[list[str]] = [
        "trp_id",
        "volume",
        "coverage",
        "is_mice",
    ...
    MEAN_SPEED_INGESTION_FIELDS: ClassVar[list[str]] = [
        "trp_id",
        "mean_speed",
        "percentile_85",
        "coverage",
    ...
    EPSG32633_REFERENCE_SYSTEM: ClassVar[int] = 32633  # WGS84

    COVID_YEARS: ClassVar[list[int]] = [2020, 2021, 2022]
    ML_CPUS: ClassVar[int] = int(
        os.cpu_count() * 0.75
    ...
    MUNICIPALITIES_AUXILIARY_DATA: ClassVar[Path] = Path(
        "data", "road_network", "kommuner.csv"
    )
    ...
    PREPROCESSING_SORTING_COLUMNS: ClassVar[list[str]] = [
        "zoned_dt_iso"
    ]  # Sorting by zoned_dt_iso since the trp_id will be removed at the end of the preprocessing phase and this lets us use the TimeSeriesSplit CV splitting method correctly
    ...
    MEAN_SPEED_TO_SCALE_FEATURES: ClassVar[list[str]] = [
        MEAN_SPEED,
        "percentile_85",
        "coverage",
    ]
...
class RowFactories(BaseModel):
    ...
    factories: ClassVar[dict[str, Any]] = {"tuple_row": tuple_row, "dict_row": dict_row}
...
class IconStyles(Enum):
    SOURCE_NODE_STYLE = {"icon_color": "#FFFFFF", "icon": "glyphicon-home"}
    DESTINATION_NODE_STYLE = {"icon_color": "#FFFFFF", "icon": "glyphicon-flag"}
    TRP_LINK_STYLE = {"icon_color": "#E7F527", "icon": "glyphicon-modal-window"}
    TOLL_STATION_LINK_STYLE = {"icon_color": "#F4550B", "icon": "glyphicon-euro"}
...
class TrafficClasses(Enum):  # Import as TrafficClasses
    ...
    CMAP = LinearSegmentedColormap.from_list(
        "TrafficClassesCMAP",
        [LOW, LOW_AVERAGE, AVERAGE, HIGH_AVERAGE, HIGH, STOP_AND_GO],
        N=256,
    )
