...
class IngestionPipelineMixin:
    ...
    async def _impute_missing_values(
        data: pd.DataFrame | dd.DataFrame, r: str = "gamma"
    ) -> pd.DataFrame:
        """
        ...
        return await asyncio.to_thread(
            lambda: pd.DataFrame(mice_imputer.fit_transform(data), columns=data.columns)
        )  # Fitting the imputer and processing all the data columns except the date one #TODO BOTTLENECK. MAYBE USE POLARS LazyFrame or PyArrow?
...
class VolumeIngestionPipeline(IngestionPipelineMixin):
    ...
    async def _get_missing(
        zoned_datetimes: set[datetime.datetime],
        ...
        return await asyncio.to_thread(
            ...
            ).difference(zoned_datetimes)
        )  # Finding all zoned datetimes which should exist (calculated above in all_dts), but that aren't withing the ones available.

    @staticmethod
    async def _get_dfs_diff_mask(
        df1: pd.DataFrame, df2: pd.DataFrame, comparison_cols: list[str]
    ):
        # Create a mask for rows in data that also exist in contextd
        return await asyncio.to_thread(
            lambda: (
                df1[comparison_cols]
                .isin(df2[comparison_cols].to_dict(orient="list"))
                .all(axis=1)
            )
        )

    async def _parse_by_hour_async(
        ...
        all(
            ...
            for m in await self._get_missing(
                set(
                    datetime.datetime.fromisoformat(node["node"]["from"]).replace(
        ...
        all(
            (
                by_hour["trp_id"].append(trp_id),
                by_hour["volume"].append(
                    edge["node"]["total"]["volumeNumbers"]["volume"]
                    if edge["node"]["total"]["volumeNumbers"] is not None
                    else None
                ...
                by_hour["coverage"].append(
                    edge["node"]["total"]["coverage"]["percentage"] or None
                ),  # For less recent data it's possible that sometimes coverage can be null, so we'll address this problem like so
                by_hour["is_mice"].append(
                    False if edge["node"]["total"]["volumeNumbers"] else True
                ),  # For less recent data it's possible that sometimes coverage can be null, so we'll address this problem like so
                by_hour["zoned_dt_iso"].append(
                    datetime.datetime.fromisoformat(edge["node"]["from"]).replace(
                        tzinfo=ZoneInfo("Europe/Oslo")
                    )
                ),
            )
        ...
        return await asyncio.to_thread(
            lambda: pd.DataFrame(by_hour).sort_values(
                by=["zoned_dt_iso"], ascending=True
            )
        )
    ...
    async def _clean_async(
        self,
        data: pd.DataFrame,
        mice_past_window: PositiveInt,
        fields: list[str] = GlobalDefinitions.VOLUME_INGESTION_FIELDS,
    ) -> pd.DataFrame | None:
        ...
        contextd = await asyncio.to_thread(
            (
                await asyncio.to_thread(
                    ...
                    [
                        ...
                        (
                            await asyncio.to_thread(
                                pd.DataFrame,
                                await self._db_broker_async.send_sql_async(
                                    sql=f"""
                            SELECT {", ".join(fields)}
        ...
        mice_treated_data = await asyncio.to_thread(
            pd.concat,
            [
                contextd[["trp_id", "is_mice", "zoned_dt_iso"]],
                await self._impute_missing_values(
                    contextd.drop(
                        columns=["trp_id", "is_mice", "zoned_dt_iso"], axis=1
                    ),
                    r="gamma",
                ),
            ],
        ...
        data = mice_treated_data[
            await self._get_dfs_diff_mask(
                mice_treated_data,
                data,
                comparison_cols=list(
                    set(fields).difference(set(GlobalDefinitions.MICE_COLS))
                ),
            )
        ]
        ...
        data["volume"] = data["volume"].astype(
            "int"
        )  # Re-converting volume to int after MICE
        data["coverage"] = data["coverage"].round(
            3
        )  # Re-converting volume to int after MICE
    ...
    async def ingest(
        self, payload: dict[str, Any], trp_id: str, fields: list[str] | None = None
    ) -> None:
        ...
        self.data = await self._clean_async(
            data=self.data,
            mice_past_window=max(1, math.ceil(len(self.data) / 2)),
            fields=fields,
        )
        if self.data is None:
        ...
        """,
            many=True,
            many_values=list(self.data.itertuples(index=False, name=None)),
        )
...
class MeanSpeedIngestionPipeline(IngestionPipelineMixin):
    ...
    async def _parse_mean_speed_async(
        self, speeds: pd.DataFrame
    ) -> pd.DataFrame | dd.DataFrame | None:
        if speeds.empty:
            return None

        speeds["coverage"] = (
            speeds["coverage"].replace(",", ".", regex=True).astype("float") * 100
        )
        speeds["mean_speed"] = (
            speeds["mean_speed"].replace(",", ".", regex=True).astype("float")
        )
        speeds["percentile_85"] = (
            speeds["percentile_85"].replace(",", ".", regex=True).astype("float")
        )

        speeds["zoned_dt_iso"] = pd.to_datetime(
            speeds["date"]
            + "T"
            + speeds["hour_start"]
            + GlobalDefinitions.NORWEGIAN_UTC_TIME_ZONE
        )
        ...
        try:
            ...
            speeds = await asyncio.to_thread(
                ...
                [
                    ...
                    await self._impute_missing_values(
                        speeds.drop(columns=["trp_id", "is_mice", "zoned_dt_iso"]),
                        r="gamma",
                    ),
                ],
                axis=1,
...
class RoadGraphObjectsIngestionPipeline:
    ...
    @staticmethod
    def _get_toll_station_property_by_id(
        properties_list: list[dict[str, Any]],
        target_id: int,
        target_property: str = "verdi",
    ) -> str | None:
        return next(
            (
                eg.get(target_property)
                for eg in properties_list
                if eg.get("id") == target_id
            ),
            None,
        )

    async def ingest_toll_stations(
        self, fp: str, batch_size: PositiveInt = 200, n_async_jobs: PositiveInt = 10
    ) -> None:
        ...
        batches = get_n_items_from_gen(
            gen=(
                (
                    station.get("id"),
                    self._get_toll_station_property_by_id(
                        properties_list=station.get("egenskaper"),
                        target_id=1078,
                        target_property="verdi",
                    ),
        ...
        async def limited_ingest(batch: list[tuple[Any]]) -> None:
            async with semaphore:
                return await self._db_broker_async.send_sql_async(
                    sql=tolls_ing_query, many=True, many_values=batch
                )

        await asyncio.gather(
            *(asyncio.wait_for(limited_ingest(batch), timeout=30) for batch in batches)
        )  # Setting a timer so if the tasks fail for whatever reason they won't hang forever
    ...
    async def ingest_nodes(
        ...
        batches = get_n_items_from_gen(
            gen=(
                (
                    ...
                    json.dumps(
                        feature.get("properties").get("legalTurningMovements", [])
                    ),
        ...
        async def limited_ingest(batch: list[tuple[Any]]) -> None:
            async with semaphore:
                return await self._db_broker_async.send_sql_async(
                    sql=ing_query, many=True, many_values=batch
                )
    ...
    async def ingest_links(
        ...
        semaphore = asyncio.Semaphore(n_async_jobs)
        road_categories = await self._db_broker_async.get_road_categories_async(
            enable_cache=True, name_as_key=True
        )
        valid_toll_station_ids = list(
            r["id"]
            for r in (
                await self._db_broker_async.send_sql_async(
        ...
        valid_trp_ids = list(
            r["id"] for r in (await self._db_broker_async.get_trp_ids_async())
        )
        ...
        link_batches = get_n_items_from_gen(
            gen=(
                (
                    feature.get("properties").get("id"),
                    feature.get("type"),
                    shape(
                        feature.get("geometry")
                    ).wkt,  # Convert geometry to WKT with shapely's shape() and then extracting the wkt
                    feature.get("properties").get("yearAppliesTo"),
                    feature.get("properties").get("candidateIds"),
                    feature.get("properties").get("roadSystemReferences"),
                    road_categories.get(
                        feature.get("properties").get("roadCategory"), None
                    ),
                    ...
                    feature.get("properties").get(
                        "trafficDirectionWrtMeteringDirection"
                    ),
        ...
        link_municipalities_matches = get_n_items_from_gen(
            gen=(
        ...
        link_counties_matches = get_n_items_from_gen(
            gen=(
        ...
        link_toll_stations_matches = get_n_items_from_gen(
            gen=(
        ...
        link_trps_matches = get_n_items_from_gen(
            gen=(
                (feature.get("properties").get("id"), station)
                for feature in links
        ...
        for query, batches in zip(
            [
                links_ing_query,
                link_municipalities_ing_query,
                link_counties_ing_query,
                link_toll_stations_ing_query,
                link_trps_ing_query,
            ],
            [
                link_batches,
                link_municipalities_matches,
                link_counties_matches,
                link_toll_stations_matches,
                link_trps_matches,
            ],
        ):
            await asyncio.gather(
                *(
                    asyncio.wait_for(limited_ingest(query, batch), timeout=30)
...
class MLPreprocessingPipeline:
    ...
    def _add_lag_features(
        data: dd.DataFrame, target: str, lags: list[PositiveInt]
    ) -> dd.DataFrame:
        data = data.reset_index(drop=True)
        for lag in lags:
            data[f"{target}_lag{lag}h"] = data.groupby("trp_id")[target].shift(
                lag
            )  # N hours shift
            # print(data.columns)
            # print(data.dtypes)
            # IMPORTANT: we're grouping because in the dataframe there's data from multiple TRPs which have completely different values
            data[f"coverage_{target}_lag{lag}h"] = (
                data["coverage"] * data[f"{target}_lag{lag}h"]
            )

        for lag_t1, lag_t2 in pairwise(lags):
            data[f"{target}_delta_{lag_t1}h_{lag_t2}h"] = (
                data[f"{target}_lag{lag_t1}h"] - data[f"{target}_lag{lag_t2}h"]
            )  # TODO IN THE FUTURE ADDRESS THE CASE WHERE THIS SUBTRACTION COULD BE ZERO
    ...
    def _scale_features(
        ...
        data: dd.DataFrame,
        feats: list[str],
        scaler: MinMaxScaler | type[callable] = MinMaxScaler,
    ) -> dd.DataFrame:
        self._scaler = scaler()
    ...
    def _encode_categorical_features(
        data: dd.DataFrame, feats: list[str]
    ) -> dd.DataFrame:
    ...
    def _add_utm_columns(df: pd.DataFrame) -> pd.DataFrame:
        ...
        easting, northing, _, _ = zip(
            *df.apply(lambda r: utm.from_latlon(r.lat, r.lon), axis=1)
        )
    ...
    def preprocess_volume(
        self, data: dd.DataFrame, lags: list[PositiveInt], z_score: bool = False
    ) -> dd.DataFrame:
        if z_score:
            data = ZScore(data, column=GlobalDefinitions.VOLUME)

        data = self._add_lag_features(
            data=data, target=GlobalDefinitions.VOLUME, lags=lags
        )
        data = self._encode_categorical_features(
            data=data, feats=GlobalDefinitions.CATEGORICAL_FEATURES
        )

        data = data.map_partitions(
            self._add_utm_columns
        )  # Convert latitude and longitude into the appropriate UTM zone eastings and northings to maintain an accurate ratio when moving across the zone and not having distortions like in WGS84 coordinates where the closer to the poles the heavier distortions come into place

        data = self._scale_features(
            data=data,
            feats=GlobalDefinitions.VOLUME_TO_SCALE_FEATURES,
            scaler=MinMaxScaler,
        )
        data = data.sort_values(
            by=GlobalDefinitions.PREPROCESSING_SORTING_COLUMNS, ascending=True
        )
        data["year"] = data["year"].astype(int)

        return data.drop(columns=GlobalDefinitions.NON_PREDICTORS, axis=1).persist()

    def preprocess_mean_speed(
        self, data: dd.DataFrame, lags: list[PositiveInt], z_score: bool = False
    ) -> dd.DataFrame:
        if z_score:
            data = ZScore(data, column=GlobalDefinitions.MEAN_SPEED)
        data = self._add_lag_features(
            data=data, target=GlobalDefinitions.MEAN_SPEED, lags=lags
        )
        data = self._encode_categorical_features(
            data=data, feats=GlobalDefinitions.CATEGORICAL_FEATURES
        )
        ...
        data = self._scale_features(
            data=data,
            feats=GlobalDefinitions.MEAN_SPEED_TO_SCALE_FEATURES,
            scaler=MinMaxScaler,
        )
        data = data.sort_values(
            by=GlobalDefinitions.PREPROCESSING_SORTING_COLUMNS, ascending=True
...
class MLPredictionPipeline:
    ...
    def _get_training_records(
        self, training_mode: Literal[0, 1], cache_latest_dt_collection: bool = True
    ) -> dd.DataFrame:
        """
        Parameters:
        ...
        def get_volume_training_data_start():
            return self._db_broker.get_volume_date_boundaries(
                trp_id_filter=trp_id_filter, enable_cache=cache_latest_dt_collection
            )["max"] - timedelta(
                hours=(
                    (
                        self._db_broker.get_forecasting_horizon(target=self._target)
                        - self._db_broker.get_volume_date_boundaries(
                            trp_id_filter=trp_id_filter,
                            enable_cache=cache_latest_dt_collection,
                        )["max"]
                    ).seconds
                    / 3600
                    * 24
        ...
        def get_mean_speed_training_data_start():
            return self._db_broker.get_mean_speed_date_boundaries(
                trp_id_filter=trp_id_filter, enable_cache=cache_latest_dt_collection
            )["max"] - timedelta(
                hours=(
                    (
                        self._db_broker.get_forecasting_horizon(target=self._target)
                        - self._db_broker.get_mean_speed_date_boundaries(
                            trp_id_filter=trp_id_filter,
                            enable_cache=cache_latest_dt_collection,
                        )["max"]
                    ).seconds
        ...
        return (
            training_functions_mapping[self._target]["loader"](
                road_category_filter=[self._road_category]
                if self._road_category
                else None,
                ...
                trp_lat=True,
                trp_lon=True,
                zoned_dt_start=training_functions_mapping[self._target][
                    "training_data_start"
                ](),
                zoned_dt_end=training_functions_mapping[self._target][
                    "date_boundaries"
                ](trp_id_filter=trp_id_filter, enable_cache=cache_latest_dt_collection)[
                    "max"
                ],
    ...
    def _generate_future_records(
        self, training_mode: Literal[0, 1], forecasting_horizon: datetime.datetime
    ) -> dd.DataFrame | None:
        """
        ...
        attr = (
            {GlobalDefinitions.VOLUME: np.nan}
            if self._target == GlobalDefinitions.VOLUME
            else {GlobalDefinitions.MEAN_SPEED: np.nan, "percentile_85": np.nan}
        )  # TODO ADDRESS THE FACT THAT WE CAN'T PREDICT percentile_85, SO WE HAVE TO REMOVE IT FROM HERE

        last_available_data_dt = (
            self._db_broker.get_volume_date_boundaries(
                trp_id_filter=tuple([self._trp_id]) if training_mode == 0 else None,
                enable_cache=False,
            )["max"]
            if self._target == GlobalDefinitions.VOLUME
            else self._db_broker.get_mean_speed_date_boundaries(
                trp_id_filter=tuple([self._trp_id]) if training_mode == 0 else None,
                enable_cache=False,
            )["max"]
        )
        ...
        rows_to_predict = pd.DataFrame(
            {
                ...
                **attr,
                ...
                "zoned_dt_iso": dt,
                "day_cos": cos_encoder(
                    dt.day, timeframe=GlobalDefinitions.DAYS_TIMEFRAME
                ),
                "day_sin": sin_encoder(
                    dt.day, timeframe=GlobalDefinitions.DAYS_TIMEFRAME
                ),
                "hour_cos": cos_encoder(
                    dt.hour, timeframe=GlobalDefinitions.HOUR_TIMEFRAME
                ),
                "hour_sin": sin_encoder(
                    dt.hour, timeframe=GlobalDefinitions.HOUR_TIMEFRAME
                ),
                "month_cos": cos_encoder(
                    dt.month, timeframe=GlobalDefinitions.MONTHS_TIMEFRAME
                ),
                "month_sin": sin_encoder(
                    dt.month, timeframe=GlobalDefinitions.MONTHS_TIMEFRAME
                ),
                "year": dt.year,
                "week_cos": cos_encoder(
                    dt.isocalendar().week, timeframe=GlobalDefinitions.WEEKS_TIMEFRAME
                ),
                "week_sin": sin_encoder(
                    dt.isocalendar().week, timeframe=GlobalDefinitions.WEEKS_TIMEFRAME
                ),
                "is_covid_year": False,
            ...
            for dt in pd.date_range(
                start=last_available_data_dt, end=forecasting_horizon, freq="1h"
            )
        )
        ...
        return (
            dd.from_pandas(rows_to_predict)
    ...
    def _get_future_records(
        self, training_mode: Literal[0, 1], lags: list[PositiveInt]
    ):
        ...
        past_data = self._get_training_records(
            training_mode=training_mode, cache_latest_dt_collection=True
        )
        future_records = self._generate_future_records(
            training_mode=training_mode,
            forecasting_horizon=self._db_broker.get_forecasting_horizon(
                target=self._target
            ),
        )

        return preprocessing_methods_mapping[self._target](
            data=dd.concat([past_data, future_records], axis="columns").repartition(
                partition_size=GlobalDefinitions.DEFAULT_DASK_DF_PARTITION_SIZE
            ),
            lags=lags,
            z_score=False,
    ...
    def start(
        ...
        if trp_tuning:
            X_tune, y_tune = split_by_target(
                data=data[data["is_future"] != True]
                .drop(columns=["is_future"])
                .persist(),
                target=self._target,
                mode=1,
            )
        ...
        X_predict, _ = split_by_target(data=data, target=self._target, mode=1)
        X_predict = X_predict[self._model.feature_order]

        data = data.reset_index(drop=True)

        data[self._target] = dd.from_array(self._model.predict(X_predict))
        data[cols_to_scale] = self._pipeline.scaler.inverse_transform(
            data[cols_to_scale]
        )
        ...
        data = data.map_partitions(
            ...
            "hour_sin",
            "hour_cos",
            GlobalDefinitions.HOUR_TIMEFRAME,
            0,
            "hour",
        )
        data = data.map_partitions(
            ...
            "day_sin",
            "day_cos",
            GlobalDefinitions.DAYS_TIMEFRAME,
            1,
            "day",
        )
        data = data.map_partitions(
            ...
            "month_sin",
            "month_cos",
            GlobalDefinitions.MONTHS_TIMEFRAME,
            1,
            "month",
        )
        data = data.map_partitions(
            ...
            "week_sin",
            "week_cos",
            GlobalDefinitions.WEEKS_TIMEFRAME,
            1,
            "week",
        )
    ...
    def plot_predictions(
        self, predictions: dd.DataFrame, show: bool = False
    ) -> plt.Figure:

        data_dates_upper_boundary = (
            self._db_broker.get_volume_date_boundaries(trp_id_filter=[self._trp_id])[
                "max"
            ]
            if self._target == GlobalDefinitions.VOLUME
            else self._db_broker.get_mean_speed_date_boundaries(
                trp_id_filter=[self._trp_id]
            )["max"]
        )
        ...
        ax.grid(axis="both")
        ax.title(
            f"{self._target} predictions from {data_dates_upper_boundary} to {self._db_broker.get_forecasting_horizon(self._target)}"
        )
